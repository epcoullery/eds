...
APP_DIR = '/var/www/eds'
VIRTUALENV_DIR = '/var/virtualenvs/eds/bin/activate'
...
def clone_remote_db():
    ...
    with cd(APP_DIR):
        with prefix('source %s' % VIRTUALENV_DIR):
            run('python manage.py dumpdata --natural-foreign --indent 1 -e auth.Permission auth cms intranet > eds.json')
        get('eds.json', '.')
    ...
    local("python ../manage.py loaddata eds.json")
